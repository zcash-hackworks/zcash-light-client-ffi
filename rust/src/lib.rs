...
pub extern "C" fn zcashlc_init_accounts_table_with_keys(
    ...
    let res = catch_panic(|| {
        ...
        for u in slice.into_iter() {
            ...
            let ufvk = UnifiedFullViewingKey::decode(&network, &ufvkstr).unwrap();
            ufvks.insert(AccountId::from(u.account_id), ufvk);
        }
...
fn ufvk_to_ffi(account_id: AccountId, ufvk: &UnifiedFullViewingKey, network: Network) -> FFIUnifiedViewingKey {
...
fn uvk_vec_to_ffi(v: Vec<FFIUnifiedViewingKey>) -> *mut FFIUVKBoxedSlice {
...
pub unsafe extern "C" fn zcashlc_derive_unified_viewing_keys_from_seed(
    ...
    let res = catch_panic(|| {
        ...
        let uvks: Vec<_> = (0..accounts)
            .map(|account| {
                UnifiedSpendingKey::from_seed(&network, &seed, AccountId::from(account))
                    .map_err(|e| {
                        format_err!("error generating unified spending key from seed: {:?}", e)
                    })
                    .map(|usk| {
                        let ufvk = usk.to_unified_full_viewing_key();
                        ufvk_to_ffi(AccountId::from(account), &ufvk, network)
                    })
            })
            .collect::<Result<_, _>>()?;
